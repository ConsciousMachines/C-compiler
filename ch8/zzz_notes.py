...
# Assignment(CompoundAssPlusEq, b, c)

# b = b + c
# tmp = Binary(plus, b, c)
# Assignment(Regular, b, tmp)


# chapter 7
# a block is more general than a compound statement, because it has declarations that last until
#   its end, so a loop is a block bc we initialize the iteration variable.
# block := a thing that contains declarations, determines scope of those declarations. 
#   (compound statement is a block, has declarations in the block)
#   (fn body is a block, has declarations in fn prototype)
#   (loops are blocks, declaration in loop initializer)
#   (if statements are blocks, in the official grammar?)
#       bc they have scope in the body, and declarations must be in the body too since declaration 
#           cant be in the conditional expression, so same as compound statement 
#       to us it doesnt matter bc IF does not have declaration in the cond exp, so its the same as compound statement

# when you enter a new block, you can still use variables from the outer scope, because 
#   that scope hasn't finished yet, so those variables still exist. 
# two variables have the same scope if their scopes end at the same point (same "}")

# loops and if-statements are "blocks" according to the official grammar, but our blocks are just a
#   compound statement, different from the official definition of "block". 

# when we process a block, we create a copy of the variable_map, and pass it to the recursive sub-tree
#   which is the compound statement. everything processed there will be added to the copy. the copy is 
#   not used again, so those declarations won't be seen anywhere else. 
#   the original variable_map was saved before we entered the block. it was not passed to the block. 
#   so it will continue to be used in the next statement, with no declarations from the block (those
#   were given to the copy which is then thrown out)
