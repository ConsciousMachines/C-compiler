...
'''
    ...
    for i, (class_name, members) in enumerate(results):
        ...
        # if no prefix, we are doing AST where Function is just a node
        # in other trees, Function is a List[] and is treated differently
        bool_is_ast:bool = False
        ...
        lines = [line1, line2]
        for member in members:
            line = f'            pretty_print({member}, depth+4)'
            lines.append(line)
        ...
        if not bool_is_ast:
            if _class_name in ['Function']:
                line3 = f'            pretty_print({members[0]}, depth+4)'
                line4 = f'            for i in {members[1]}: # body:List[]'
                line5 = f'                pretty_print(i, depth+4)'
                lines = [lines[0], lines[1], line3, line4, line5]
